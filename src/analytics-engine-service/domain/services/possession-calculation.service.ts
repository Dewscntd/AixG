...
export const calculatePossessionByPeriods = (
  ...
  return Array.from({ length: numberOfPeriods }, (_, index) => {
    ...
    const periodEnd = Math.min(
      (index + 1) * periodDurationSeconds,
      totalDuration
    );

    const periodSequences = sequences
      .filter(seq => seq.startTime < periodEnd && seq.endTime > periodStart)
      .map(seq => ({
        ...seq,
...
export const calculatePossessionByZone = (
  ...
  FIELD_ZONES.map(zone => {
    const zoneSequences = sequences.filter(
      seq =>
        seq.teamId === teamId &&
        seq.events.some(event => isPositionInZone(event.position, zone))
    );

    const zoneDuration = zoneSequences.reduce((total, seq) => {
    ...
    const totalTeamDuration = calculateTeamPossessionDuration(
      sequences,
      teamId
    );
    const zonePercentage =
      totalTeamDuration > 0 ? (zoneDuration / totalTeamDuration) * 100 : 0;
