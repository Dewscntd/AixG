    ...
    GraphQLModule.forRootAsync<ApolloGatewayDriverConfig>({
      ...
      useFactory: async (
        ...
        const redis = new Redis(
          configService.get<string>('gateway.redisUrl') ||
        ...
        const subgraphs = [
          {
            name: 'analytics',
            url:
              configService.get<string>('gateway.analyticsServiceUrl') ||
              'http://localhost:3000/graphql',
          },
          {
            name: 'video-ingestion',
            url:
              configService.get<string>('gateway.videoIngestionServiceUrl') ||
              'http://localhost:3001/graphql',
          },
          {
            name: 'ml-pipeline',
            url:
              configService.get<string>('gateway.mlPipelineServiceUrl') ||
              'http://localhost:8000/graphql',
          },
          {
            name: 'team-management',
            url:
              configService.get<string>('gateway.teamManagementServiceUrl') ||
        ...
        return {
          ...
          server: {
            ...
            context: async ({
              ...
              if (connection) {
                ...
                return {
                  ...
                  redis,
                  correlationId: `ws-${Date.now()}-${Math.random()
                    .toString(36)
                    .substr(2, 9)}`,
                  startTime: Date.now(),
                };
            ...
            plugins: [
              ...
              new ComplexityPlugin({
                ...
                createError: (max, actual) =>
                  new Error(
                    `Query complexity ${actual} exceeds maximum ${max}`
                  ),
              }),
            ],
            ...
            formatError: (error: any) => {
              ...
              if (
                configService.get<string>('gateway.nodeEnv') === 'production'
              ) {
                return {
            ...
            formatResponse: (
              response: any,
              { request: _request, context }: { request: any; context: any }
            ) => {
