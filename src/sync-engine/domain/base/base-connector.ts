...
import { SyncResult, SyncError, SyncWarning } from '../value-objects/sync-result';
...
export abstract class BaseConnector implements IConnector {
  ...
  async connect(configuration: SyncConfiguration): Promise<ConnectionStatus> {
    try {
      ...
      throw new Error(`Failed to connect to ${this.getSystemType().value}: ${error}`);
    }
  }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      console.error(`Error during disconnection from ${this.getSystemType().value}:`, error);
  ...
  async healthCheck(): Promise<boolean> {
    try {
      ...
      console.error(`Health check failed for ${this.getSystemType().value}:`, error);
      return false;
    }
  }
   ...
   */
  async sync(dataTypes: DataTypeEnum[], lastSyncTimestamp?: Date): Promise<SyncResult> {
    ...
    const syncResults: { dataType: DataTypeEnum; success: boolean; records: number; error?: SyncError }[] = [];
    ...
    for (const dataType of dataTypes) {
      try {
        ...
        const { recordsProcessed, recordsCreated, recordsUpdated, recordsSkipped, warnings: typeWarnings } = 
        ...
        if (syncError.recoverable && this.retryCount < this.configuration!.retryAttempts) {
          this.retryCount++;
          console.log(`Retrying ${dataType} sync (attempt ${this.retryCount}/${this.configuration!.retryAttempts})`);
    ...
    const totalRecordsProcessed = successfulResults.reduce((sum, r) => sum + r.records, 0);
   ...
   */
  async validateConfiguration(configuration: SyncConfiguration): Promise<boolean> {
    try {
      ...
      console.error(`Configuration validation failed for ${this.getSystemType().value}:`, error);
      return false;
    }
  }
   ...
   */
  getRateLimit(): { requestsPerMinute: number; burstSize: number; cooldownPeriod: number } {
    return {
  ...
  protected abstract validateSpecificConfiguration(configuration: SyncConfiguration): Promise<boolean>;
  ...
  protected isRecoverableError(error: any): boolean {
    ...
    const recoverableCodes = ['ECONNRESET', 'ETIMEDOUT', 'ENOTFOUND', '429', '503', '502'];
