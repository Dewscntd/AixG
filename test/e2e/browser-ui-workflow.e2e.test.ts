...
test.describe('FootAnalytics UI Workflow E2E', () => {
  ...
  test('complete video upload and analysis workflow', async () => {
    ...
    await test.step('Fill match details', async () => {
      ...
      await page.selectOption('[data-testid="match-competition"]', matchData.competition);
    });
    ...
    await test.step('Monitor processing status', async () => {
      ...
      await expect(matchRow.locator('[data-testid="status"]')).toHaveText('מעבד...');
      
      // Wait for processing to complete (with reasonable timeout for tests)
      await expect(matchRow.locator('[data-testid="status"]')).toHaveText('הושלם', {
    ...
    await test.step('View analytics dashboard', async () => {
      ...
      await expect(page.locator('[data-testid="xg-label"]')).toContainText('xG');
      await expect(page.locator('[data-testid="possession-label"]')).toContainText('החזקה');
      ...
      await expect(page.locator('[data-testid="possession-home"]')).toBeVisible();
      await expect(page.locator('[data-testid="possession-away"]')).toBeVisible();
    });
    ...
    await test.step('Interact with visualizations', async () => {
      ...
      await expect(page.locator('[data-testid="player-heatmap"]')).toBeVisible();
      
      // Test shot map
      await page.click('[data-testid="shot-map-tab"]');
      await expect(page.locator('[data-testid="shot-map-canvas"]')).toBeVisible();
    ...
    await test.step('Test video player integration', async () => {
      ...
      const timelineEvent = page.locator('[data-testid^="timeline-event-"]').first();
  ...
  test('real-time analysis workflow', async () => {
    await test.step('Start live analysis', async () => {
      ...
      await expect(page.locator('[data-testid="live-analysis-setup"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="live-video-feed"]')).toBeVisible();
    });

    await test.step('Monitor real-time analytics', async () => {
      // Wait for first analytics update
      await expect(page.locator('[data-testid="live-xg-home"]')).toBeVisible({ timeout: 10000 });
      await expect(page.locator('[data-testid="live-xg-away"]')).toBeVisible();
      
      // Check live possession updates
      await expect(page.locator('[data-testid="live-possession"]')).toBeVisible();
      
      // Verify player tracking overlay
      await expect(page.locator('[data-testid="player-tracking-overlay"]')).toBeVisible();
    });
    ...
    await test.step('Save live session', async () => {
      ...
      await page.fill('[data-testid="session-notes"]', 'מחצית ראשונה - ביצועים טובים');
  ...
  test('analytics comparison and export', async () => {
    await test.step('Navigate to comparison view', async () => {
      ...
      await expect(page.locator('[data-testid="comparison-dashboard"]')).toBeVisible();
    });
    ...
    await test.step('View comparison charts', async () => {
      // Check xG comparison
      await expect(page.locator('[data-testid="xg-comparison-chart"]')).toBeVisible();
      
      // Check possession comparison
      await expect(page.locator('[data-testid="possession-comparison-chart"]')).toBeVisible();
      
      // Check player performance comparison
      await page.click('[data-testid="player-comparison-tab"]');
      await expect(page.locator('[data-testid="player-comparison-table"]')).toBeVisible();
    });
  ...
  test('mobile responsive design', async () => {
    ...
    await test.step('Test tablet viewport', async () => {
      ...
      // Check tablet layout
      await expect(page.locator('[data-testid="analytics-grid"]')).toBeVisible();
