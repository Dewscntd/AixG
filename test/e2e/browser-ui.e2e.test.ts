...
test.describe('FootAnalytics UI E2E Tests', () => {
  ...
  test.describe('Video Upload Interface', () => {
    ...
    test('should show upload progress', async () => {
      ...
      await page.route('**/api/videos/upload', route => {
        route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ videoId: 'test-video-id', status: 'uploading' })
        });
      });
    ...
    test('should validate file format', async () => {
      ...
      await expect(page.locator('[data-testid="file-format-error"]')).toContainText('Invalid file format');
    });
  });
  ...
  test.describe('Analytics Dashboard', () => {
    ...
    test('should display match analytics correctly', async () => {
      ...
      await expect(page.locator('[data-testid="home-team-xg"]')).toContainText('2.15');
      await expect(page.locator('[data-testid="away-team-xg"]')).toContainText('1.87');
      await expect(page.locator('[data-testid="home-team-possession"]')).toContainText('62.4%');
      await expect(page.locator('[data-testid="away-team-possession"]')).toContainText('37.6%');
    });
    ...
    test('should switch between different analytics views', async () => {
      ...
      await expect(page.locator('[data-testid="formation-diagram"]')).toBeVisible();
    });
  ...
  test.describe('Real-time Analytics', () => {
    ...
    test('should update analytics in real-time', async () => {
      ...
      await page.evaluate(() => {
        ...
        window.dispatchEvent(new CustomEvent('analytics_update', { detail: mockUpdate }));
      });
      
      await expect(page.locator('[data-testid="live-xg-home"]')).toContainText('1.25');
      await expect(page.locator('[data-testid="live-xg-away"]')).toContainText('0.87');
    });
  ...
  test.describe('Hebrew RTL Support', () => {
    test('should display Hebrew interface correctly', async () => {
      ...
      await expect(page.locator('[data-testid="dashboard-nav"]')).toContainText('לוח בקרה');
      await expect(page.locator('[data-testid="analytics-nav"]')).toContainText('אנליטיקה');
    });
