...
import {
  ...
  calculateBatchXG,
  calculateTotalXG,
} from '../../../src/analytics-engine-service/domain/services/xg-calculation.service';
import {
  FootballArbitraries,
  PropertyTestUtils,
} from '../../setup/property.setup';
...
describe('XGCalculationService - Property-Based Tests', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (baseShotData, distanceReduction) => {
            ...
            const closerShot = {
              ...baseShotData,
              distanceToGoal: Math.max(
                1,
                baseShotData.distanceToGoal - distanceReduction
              ),
            };
            ...
            expect(closerXG.value).toBeGreaterThanOrEqual(
              originalXG.value - 0.1
            );
          }
        )
      );
      ...
      fc.assert(
        fc.property(
          fc.record({
            ...
            bodyPart: fc.constantFrom(
              'foot' as const,
              'head' as const,
              'other' as const
            ),
            situation: fc.constantFrom(
              'penalty' as const,
              'open_play' as const
            ),
    ...
    it('should maintain composition equivalence', () => {
      fc.assert(
        fc.property(FootballArbitraries.shotData(), shotData => {
          ...
          expect(
            Math.abs(standardResult.value - composedResult.value)
          ).toBeLessThan(0.0001);
        })
      );
    });
  ...
  describe('Batch Processing Properties', () => {
    it('should maintain consistency between single and batch calculations', () => {
      fc.assert(
        fc.property(
          fc.array(FootballArbitraries.shotData(), {
            minLength: 1,
            maxLength: 10,
          }),
          shotDataArray => {
            ...
            batchResults.forEach((batchResult, index) => {
              ...
              expect(
                Math.abs(batchResult.value - individualResult!.value)
              ).toBeLessThan(0.0001);
            });
          }
        )
    ...
    it('should maintain total consistency between batch and individual calculations', () => {
      fc.assert(
        fc.property(
          ...
          shotDataArray => {
            const batchResults = calculateBatchXG(shotDataArray);
            const totalFromBatch = batchResults.reduce(
              (sum, xg) => sum + xg.value,
              0
            );
  ...
  describe('Domain-Specific Properties', () => {
    it('should give penalties very high xG', () => {
      fc.assert(
        fc.property(FootballArbitraries.shotData(), baseShotData => {
          const penaltyShot = {
            ...baseShotData,
    ...
    it('should handle defender impact correctly', () => {
      fc.assert(
        fc.property(FootballArbitraries.shotData(), baseShotData => {
          ...
          expect(noDefendersXG.value).toBeGreaterThanOrEqual(
            manyDefendersXG.value
          );
        })
      );
    });
      ...
      fc.assert(
        fc.property(
          ...
          shots => {
            ...
            const totalXG = individualXGs.reduce(
              (sum, xg) => sum + xg.value,
              0
            );
      ...
      fc.assert(
        fc.property(
          ...
          shots => {
            ...
            const openPlayShots = shots.filter(
              shot => shot.situation === 'open_play'
            );
            const closeShots = openPlayShots.filter(
              shot => shot.distanceToGoal <= 6 && shot.defenderCount <= 1
            );
            const farShots = openPlayShots.filter(
              shot => shot.distanceToGoal >= 25 && shot.defenderCount <= 1
            );

            if (closeShots.length > 0 && farShots.length > 0) {
              const avgCloseXG =
                closeShots.reduce(
                  (sum, shot) => sum + calculateXG(shot).value,
                  0
                ) / closeShots.length;
              const avgFarXG =
                farShots.reduce(
