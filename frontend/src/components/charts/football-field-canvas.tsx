 ...
 * Football Field Canvas Component
 */
export const FootballFieldCanvas = forwardRef<FootballFieldCanvasRef, FootballFieldCanvasProps>(
  ({
    width = 800,
    height = 520,
    className,
    ...
    const [animatedPlayers, setAnimatedPlayers] = useState<PlayerPosition[]>(players);
    ...
    const drawField = useCallback((ctx: CanvasRenderingContext2D) => {
      const { width: fieldWidth, height: fieldHeight, offsetX, offsetY } = fieldDimensions;
      ...
      // Penalty areas
      const { penaltyAreaWidth, penaltyAreaHeight, goalAreaWidth, goalAreaHeight } = fieldDimensions;
      ...
      // Penalty spots
      ctx.beginPath();
      ctx.arc(offsetX + penaltyAreaWidth * 0.65, offsetY + fieldHeight / 2, 2, 0, 2 * Math.PI);
      ctx.fill();
      
      ctx.beginPath();
      ctx.arc(offsetX + fieldWidth - penaltyAreaWidth * 0.65, offsetY + fieldHeight / 2, 2, 0, 2 * Math.PI);
      ctx.fill();
      
      // Goals
      ctx.lineWidth = 3;
      ctx.strokeRect(offsetX - 8, offsetY + fieldHeight * 0.4, 8, fieldHeight * 0.2);
      ctx.strokeRect(offsetX + fieldWidth, offsetY + fieldHeight * 0.4, 8, fieldHeight * 0.2);
    ...
    }, [showPlayers, animatedPlayers, fieldDimensions, homeTeamColor, awayTeamColor, colors.text]);
    ...
    }, [canvasContext, drawField, drawHeatmap, drawPlayers, drawEvents, performanceMonitor]);
    ...
    const handleCanvasClick = useCallback((event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      // Field click
      if (onFieldClick && fieldX >= 0 && fieldX <= 100 && fieldY >= 0 && fieldY <= 100) {
        onFieldClick(fieldX, fieldY);
      }
    }, [fieldDimensions, animatedPlayers, animatedEvents, onPlayerClick, onEventClick, onFieldClick]);
